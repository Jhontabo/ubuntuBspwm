...
if [ -n "$BATTERY_NAME" ] && [ -n "$ADAPTER_NAME" ]; then
  ...
  export POLYBAR_RIGHT_MODULES="wifi sep volume sep cpu sep ram sep battery sep date sep powermenu sep tray"
else
  unset POLYBAR_BATTERY POLYBAR_ADAPTER
  export POLYBAR_RIGHT_MODULES="wifi sep volume sep cpu sep ram sep date sep powermenu sep tray"
fi
