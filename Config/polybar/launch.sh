...
if [ -n "$BATTERY_NAME" ] && [ -n "$ADAPTER_NAME" ]; then
  ...
  export POLYBAR_RIGHT_MODULES="date sep volume sep keyboard sep cpu sep ram sep battery sep wifi sep nightlight sep powermenu sep tray"
else
  unset POLYBAR_BATTERY POLYBAR_ADAPTER
  export POLYBAR_RIGHT_MODULES="date sep volume sep keyboard sep cpu sep ram sep wifi sep nightlight sep powermenu sep tray"
fi
